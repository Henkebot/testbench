	...
	, m_bHasSRV(false)
{
	...
	m_pVertexBufferUpload->SetName(L"VertexUploadHeap");
}
...
void VertexBufferDX12::_createSRV(size_t _ElementSize)
{
	...
	m_pRender->GetDevice()->CreateShaderResourceView(
		m_pVertexBufferUpload.Get(), &srvDesc, srvHandle0);
	m_bHasSRV = true;
}
...
void VertexBufferDX12::setData(const void* _data, size_t _size, size_t offset)
{
	...
	ThrowIfFailed(
		m_pVertexBufferUpload->Map(0, &readRange, reinterpret_cast<void**>(&pVertexDataBegin)));
